...
#define ECHO_PIN 2
#define MOTOR_SERVO 8
#define POTENTIOMETER_PIN A0
#define SENSOR_TEMPERATURA A1

LiquidCrystal lcd(2, 3, 4, 5, 6, 7); // Pines de conexión a la LCD (RS, E, D4, D5, D6, D7)
...
void loop() 
{
  ...
  else 
  {
    ...
    lcd.clear(); // Limpiar la pantalla LCD cuando la luz amarilla está apagada
  }
}
...
bool controlarTemperatura() 
{
  ...
  if (temperaturaCelsius >= 37.5) 
  {
    ...
    digitalWrite(LED_VERDE, LOW);
    mostrarEnLCD(String(temperaturaCelsius), "Denegado");
    return false;
  } 
  else 
  {
    ...
    digitalWrite(LED_ROJA, LOW);
    mostrarEnLCD(String(temperaturaCelsius), "Permitido");
    return true;
  }
}
...
void mostrarEnLCD(String temperaturaCelsius, String linea2) 
{
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Temp: " + temperaturaCelsius + " C");
  lcd.setCursor(0, 1);
  lcd.print(linea2);
}
